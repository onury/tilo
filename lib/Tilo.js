...
const disabledChalk = new chalk_1.default.constructor({ enabled: false });
...
class Tilo extends EventEmitter {
    ...
    set styles(value) {
        ...
        this.$.chalk = value ? chalk_1.default : disabledChalk;
    }
    /**
    ...
    /**
     *  Gets the Chalk instance used for styling.
     ...
     *  @param {LogLevel} level - Log level to be used.
     *  @param {...any} args - Arguments to be logged.
     *
     *  @example
     ...
     *  @protected
     *  @param {LogLevel} level - Level of the given log.
    ...
    $write(logInfo, useFormatter = true) {
        ...
        if (levelEnabled && stream) {
            ...
            const log = useFormatter
                ? (this.format || helper_1.helper.identityFormat).apply(null, [logInfo, this.chalk])
                : logInfo.text;
