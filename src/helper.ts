...
// own modules
import type { ILogInfo, ILogLevelStreams } from './index.js';
...
const helper = {
  ...
  isWritableObject(o: any): boolean {
    return typeof o === 'object' && typeof (o as any).write === 'function';
  },
  ...
  safeStringify(o: any, pretty?: boolean): string {
    // fast-safe-stringify is CJS (`module.exports = fn`); under NodeNext its
    // `export default` typings model the value as a namespace, so we cast to
    // the callable signature. At runtime the default import is the function.
    const stringifyFn = stringify as unknown as (
      value: any,
      replacer?: any,
      space?: string | number
    ) => string;
    return stringifyFn(o, null, pretty ? 2 : 0);
  },
